...
const ChatPanel = ({ messages, onSendMessage, onChatMessage, isGenerating = false, onNewProject, streamingText = '', projectId, onAnalyze }) => {
  ...
  return (
    <div className="w-96 border-r border-gray-800 flex flex-col bg-[#0f0f0f]" data-testid="chat-panel">
      ...
      <div className="p-3 border-b border-gray-800">
        ...
        <div className="flex bg-gray-900 rounded-lg p-0.5">
          <button
            ...
            className={`flex-1 text-xs py-1.5 rounded-md flex items-center justify-center space-x-1 transition-all ${
          ...
          <button
            ...
            className={`flex-1 text-xs py-1.5 rounded-md flex items-center justify-center space-x-1 transition-all ${
              mode === 'chat' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
      ...
      {showLowCreditsNudge && (
        <div
          className="mx-3 mt-3 p-3 rounded-xl border border-amber-500/40 bg-gradient-to-br from-amber-500/10 to-orange-500/5 flex items-center gap-3"
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs text-amber-100 leading-tight">
              ...
              <span className="text-amber-200/80"> — top up anytime</span>
            </p>
            <button
              onClick={() => navigate('/pricing')}
              className="text-xs font-semibold text-white hover:text-amber-100 underline underline-offset-2 mt-0.5"
              data-testid="low-credits-nudge-cta"
            >
              Buy credits →
            </button>
          </div>
          <button
            ...
            data-testid="low-credits-nudge-dismiss"
            title="Dismiss"
          >
            <X className="w-4 h-4" />
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="py-4" data-testid="chat-empty-state">
            <div className="text-center mb-6">
              <Sparkles className="w-7 h-7 text-blue-500/40 mx-auto mb-2" />
              <p className="text-sm text-gray-400">
                {mode === 'build' ? 'What do you want to build?' : 'Ask about your project'}
              </p>
            </div>
            {mode === 'build' && (
              <div data-testid="template-gallery">
                <p className="text-[11px] uppercase tracking-wider text-gray-600 mb-3 px-1">Quick Start Templates</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { label: 'Portfolio', prompt: 'portfolio website with project gallery, skills, timeline', icon: '🎨' },
                    { label: 'E-Commerce', prompt: 'e-commerce store with products, cart, checkout', icon: '🛒' },
                    { label: 'SaaS Landing', prompt: 'SaaS landing page with pricing, features, testimonials', icon: '🚀' },
                    { label: 'Blog Platform', prompt: 'blog platform with articles, categories, authors', icon: '📝' },
                    { label: 'Fitness Tracker', prompt: 'fitness tracker with workouts, progress, exercises', icon: '💪' },
                    { label: 'Restaurant', prompt: 'restaurant website with menu, reservations, gallery', icon: '🍽' },
                    { label: 'Task Manager', prompt: 'task manager with boards, priorities, due dates', icon: '📋' },
                    { label: 'Finance App', prompt: 'finance dashboard with transactions, budgets, charts', icon: '💰' },
                  ].map((t) => (
                    <button
                      key={t.label}
                      ...
                      disabled={isGenerating}
                      className="text-left p-3 rounded-xl bg-gray-900/50 border border-gray-800 hover:border-gray-700 hover:bg-gray-800/50 transition-all group"
                    >
                      <span className="text-lg">{t.icon}</span>
                      <p className="text-xs font-medium text-gray-300 mt-1 group-hover:text-white transition-colors">{t.label}</p>
                    </button>
                  ))}
                </div>
              </div>
        ...
        {messages.map((message) => (
          <div
            key={message.id}
            data-testid={`chat-msg-${message.type}`}
          ...
          >
            <div
              className={`max-w-[90%] rounded-xl p-3 ${
                message.type === 'user'
                  ...
                  : message.isAnalysis
                  ? 'bg-gradient-to-br from-orange-900/30 to-amber-900/20 border border-orange-800/30 text-gray-100 rounded-bl-sm'
            ...
            >
                ...
                <div className="flex items-center space-x-2 mb-2">
                  {message.isAnalysis ? (
                    <>
                      ...
                      <span className="text-[11px] text-orange-400 font-medium">Project Audit</span>
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-3.5 h-3.5 text-blue-400" />
                      <span className="text-[11px] text-gray-400">Nexa AI</span>
                    </>
                  )}
                </div>
              )}
              ...
              {message.actions && !message.isStreaming && message.actions.length > 0 && (
